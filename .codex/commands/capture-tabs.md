...
# /capture-tabs — Codex AI Research Capture

Follow [[AGENTS.md]] and read [[Core Context]] once per session. This command is the Codex mirror of `.claude/commands/capture-tabs.md`; it is a pre-ingest capture layer for research gathered through ChatGPT, Gemini, Grok, Claude, Perplexity, and adjacent source tabs.
...
3. Put Codex interpretation under `## Agent Capture Notes`, never inside `## Original Content`.
...
1. Existing Chrome tab group:
	- Use Computer Use when the task requires operating the user's visible Chrome UI or account-bound ChatGPT/Gemini/Grok/Claude/Perplexity tabs.
	- If macOS or Chrome automation is available, first collect title/URL/tab metadata through a non-destructive read.
2. In-app browser or normal web page:
	- Use the Browser plugin when the target is already in the Codex in-app browser, local browser surface, or a URL Codex can open independently.
3. Fallback:
...
2. If allowed and available, use browser/Chrome text extraction for `document.body.innerText`.
3. If UI extraction is the only path, use Computer Use to select/copy visible conversation text.
...
- `run-inbox`: run `.codex/commands/inbox.md` on the new file.
- `ingest-now`: run `.codex/commands/ingest.md` after the collection-purpose gate and mothership link search.
